...
│   ├── deploy/            ← Deploy pipeline scripts (Python)
│   │   ├── copy-assets.py          ← Copy source/binary/walkthrough from projects
│   │   ├── generate-play-pages.py  ← Generate standalone play.html pages
│   │   └── generate-landing-pages.py ← Generate per-game landing pages
...
└── ifhub/                 ← IF Hub web player
    ...
    ├── games.json         ← Game registry (titles, binaries, sound flags, cards, deploy paths)
...
The hub at `ifhub/` aggregates all games. Adding a new game requires updating only `games.json` — the `deploy` object in each entry specifies source, binary, and walkthrough paths. `deploy.sh` reads these fields via `tools/deploy/copy-assets.py` (no hardcoded arrays). See `docs/functional-spec.md` section 6 for full deploy pipeline details.
...
Projects with multiple playable milestones can use a `versions/` directory to store frozen snapshots. Tools: `snapshot.sh` (freeze/update), `build-site.sh` (assemble `_site/` from `web/` + `versions/`). GitHub Actions assembles `_site/` inline for deployment. The `_site/` directory is gitignored.
