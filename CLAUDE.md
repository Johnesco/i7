...
│   │   └── extract-cards.py        ← Extract card metadata from project landing pages → cards.json
...
└── ifhub/                 ← IF Hub web player
    ...
    ├── games.json         ← Game registry (titles, binaries, sound flags, landing paths, deploy paths)
    ├── cards.json         ← Card metadata extracted from landing pages (generated by deploy.sh)
    ├── deploy.sh          ← Gathers assets + landing pages from projects/, generates cards.json
