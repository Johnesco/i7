...
Generated by `deploy.sh` (via `tools/deploy/generate-landing-pages.py`) from `ifhub/landing-template.html` for games with a `card` object (unless `customLanding: true`).
...
**Landing pages** (`tools/deploy/generate-landing-pages.py`):
- Generates `games/<base>/index.html` from `ifhub/landing-template.html` for games with a `card` (excluding `customLanding: true`)
...
### 6.3 Post-Deploy Validation (`tools/validate-web.sh`)
...
Adding a new game requires updating **only `games.json`**:

1. Add a game entry with `id`, `title`, `binary`, `source` (hub-relative paths for the frontend)
2. Add a `deploy` object with `source`, `binary`, and `walkthroughDir` (project-relative paths for asset copying)
3. Optionally add a `card` object for the landing page, `playTemplate` for custom templates, etc.
...
**Post-build validation:** After `setup-web.sh` completes, `compile.sh` runs `validate-web.sh` on the generated `web/` directory to catch common deployment issues (missing files, broken template tokens, malformed binaries). See section 6.3 for the full list of checks.
